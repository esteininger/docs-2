...
To do this, store a version identifier in the graph's state. On the first run for a thread, an entry node stamps the current version. Because state is [checkpointed](/oss/python/langgraph/persistence), the version persists across runs. A conditional edge then routes to the correct implementation:
