...
Today developers can choose how they build agents: [LangChain](/oss/python/langchain/overview) for flexibility and control, or [Deep Agents](/oss/python/langchain/overview) for a batteries-included experience with built-in planning, filesystem tools, subagents, and context management. Both are built on [LangGraph](/oss/python/langgraph/overview).
