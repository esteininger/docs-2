...
LangChain and [Deep Agents](/oss/python/langchain/overview) provide prebuilt middleware for common use cases. Each middleware is production-ready and configurable for your specific needs.
...
=======
`FilesystemMiddleware` from [deep agents](/oss/python/langchain/overview) provides four tools for interacting with both short-term and long-term memory:
>>>>>>> cd326bc6f (Fix links)
...
The subagents middleware from [deep agents](/oss/python/langchain/overview) allows you to supply subagents through a `task` tool.
