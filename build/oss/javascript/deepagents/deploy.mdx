...
The [Deep Agents CLI](/oss/javascript/deepagents/cli/overview) includes a `deploy` command that packages and deploys your agent to [LangSmith Deployment](/langsmith/deployment) in a single step. Define your agent's configuration in a `deepagents.toml` file and deploy directly from your project directory.
...
| --- | --- | --- |
| `AGENTS.md` | [Memory](/oss/javascript/deepagents/customization#memory) for the agent. Provides persistent context (project conventions, instructions, preferences) that is always loaded at startup. | Yes |
...
Core agent identity. For more on model selection and provider configuration, see [models](/oss/javascript/deepagents/models).
...
For the full list of providers and configuration details, see [models](/oss/javascript/deepagents/models).
