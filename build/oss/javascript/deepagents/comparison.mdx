...
- **Bring your own infrastructure**: No built-in deployment infrastructure. Production use requires provisioning your own containers, building your own HTTP/WebSocket layer, and wiring up auth, monitoring, and scaling.
...
Deep Agents includes deployment infrastructure out of the box. Running `deepagents deploy` packages your agent and deploys it as a [LangSmith Deployment](/langsmith/deployment): a horizontally scalable server with built-in multi-tenant authentication, open-protocol endpoints, webhooks, cron, and LangSmith observability. Deployments are self-hostable.

Claude Agent SDK does not include deployment infrastructure. To serve it in production, you provision your own containers, build your own HTTP/WebSocket layer, and wire up auth, monitoring, and scaling yourself. There is no built-in multi-tenant authentication, webhook support, cron scheduling, or observability.

| | **Deep Agents** | **Claude Agent SDK** |
| --- | --- | --- |
| Deployment approach | `deepagents deploy` (one command) | Build your own HTTP/WebSocket layer |
| Multi-tenant auth | ✅ [Built-in](/langsmith/auth) | ❌ |
| Webhooks | ✅ | ❌ |
| Cron | ✅ | ❌ |
| Standard protocol endpoints | MCP, A2A, Agent Protocol | Proprietary |
| Self-hosting | ✅ | ✅ (container-based) |
| Observability | ✅ [LangSmith](/langsmith/home) | ❌ |

For more detail, see [Deploy with the CLI](/oss/javascript/deepagents/deploy) and [Going to production](/oss/javascript/deepagents/going-to-production).
...
| A2A endpoint | ✅ [A2A endpoint in Agent Server](https://docs.langchain.com/langsmith/server-a2a#a2a-endpoint-in-agent-server) | ✅ (third-party) | ❌ |
