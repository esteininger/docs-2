...
- **Local-first design**: Built on top of Claude Code, a local CLI tool. Production deployment requires building and maintaining your own HTTP/WebSocket layer and running the SDK in containers.
...
Claude Agent SDK is built on top of Claude Code, a local CLI tool, and is designed primarily for local development. To serve it in production, you build and maintain your own HTTP/WebSocket layer and run the SDK in containers. There is no built-in multi-tenant authentication, webhook support, cron scheduling, or observability.

DeepAgents is designed for production from the start. Running `deepagents deploy` packages your agent and deploys it as a [LangSmith Deployment](/langsmith/deployment): a horizontally scalable server with built-in multi-tenant authentication, open-protocol endpoints, webhooks, cron, and LangSmith observability. Deployments are self-hostable.
