...
def post_run(run_id, name, run_type, inputs, parent_id=None):
    ...
    requests.post(
        "https://api.smith.langchain.com/runs",  # Update for self-hosted, EU (`eu.api...`), or AWS US (`aws.api...`)
        json=data,
        headers=headers
    )
...
api_url = "https://api.smith.langchain.com"  # Or eu.api / aws.api for regional SaaS
