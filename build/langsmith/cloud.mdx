...
The [AWS-hosted LangSmith service](https://aws.smith.langchain.com) runs in AWS `us-east-2` (Ohio). The LangSmith control plane (UI, APIs, and related services) runs on Amazon EKS. [LangSmith Cloud deployments](/langsmith/deployment) for that environment are provisioned in a separate LangGraph dataplane cluster in the same AWS region; deployment hostnames use the `*.aws.us.langgraph.app` domain (see the [Regions FAQ](/langsmith/regions-faq)).

#### Regional storage

The resources and services in this table are stored in the location corresponding to the URL where sign-up occurred (US GCP, EU GCP, or AWS US). Cloud-managed LangSmith uses [Supabase](https://supabase.com) for authentication/authorization and [ClickHouse Cloud](https://clickhouse.com/cloud) for the data warehouse.

|                                     | US (GCP)                                                           | EU (GCP)                                                                 | AWS (US)                                                                 |
| ----------------------------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------------ | ------------------------------------------------------------------------ |
...
| Supabase                            | AWS us-east-1 (N. Virginia)                                        | AWS eu-central-1 (Germany)                                               | Hosted auth at `aws.auth.langchain.com` (Supabase-backed)                |
| ClickHouse Cloud                    | us-central1 (Iowa)                                                 | europe-west4 (Netherlands)                                               | ClickHouse Cloud over AWS PrivateLink in `us-east-2`                     |
...
The following applies to the **US and EU** SaaS regions on GCP. The **AWS** SaaS region uses the same logical components on **Amazon EKS** (and related AWS networking and data services) instead of GKE.
...
| US (GCP)       | EU (GCP)       | AWS (control plane, us-east-2) |
| -------------- | -------------- | ------------------------------ |
| 34.59.65.97    | 34.13.192.67   | _Fill: NAT gateway IP 1_       |
| 34.67.51.221   | 34.147.105.64  | _Fill: NAT gateway IP 2_       |
| 34.46.212.37   | 34.90.22.166   | _Fill: NAT gateway IP 3_       |
...
<Note>
**AWS column:** Replace the placeholders with the public IPs of the LangSmith control-plane NAT gateways in `us-east-2` (one per availability zone in use). Your LangChain account team can provide the current list, or they are emitted as `langsmith_prod_nat_gateway_public_ips` from internal deployment tooling.
</Note>
...
For **egress from LangGraph Cloud deployments** on AWS (traffic leaving the LangGraph dataplane VPC toward your endpoints), use the separate NAT allowlist in [Allowlist IP addresses](/langsmith/deploy-to-cloud#allowlist-ip-addresses) (AWS / `us-east-2` column).

#### Ingress into LangChain SaaS
The LangChain endpoints map to the following static IP addresses for traffic that terminates on our **GCP load balancers** (US/EU) or on the **AWS Network Load Balancer** (API and gateway hostnames for the AWS region):

| US (GCP)       | EU (GCP)       | AWS (API / gateway NLB, us-east-2) |
| -------------- | -------------- | ---------------------------------- |
| 34.8.121.39    | 34.95.92.214   | _Fill: NLB Elastic IP 1_           |
| 34.107.251.234 | 34.13.73.122   | _Fill: NLB Elastic IP 2_           |
|                |                | _Fill: NLB Elastic IP 3_           |

<Note>
**AWS column:** Replace the placeholders with the stable Elastic IPs attached to the internet-facing Envoy Gateway NLB for `aws.api.smith.langchain.com` and related API hosts (`aws.api.hub.langchain.com`, `aws.api.host.langchain.com`, etc.). Your LangChain account team can provide the current list, or they are emitted as `langsmith_prod_envoy_gateway_nlb_public_ips` from internal deployment tooling.
</Note>

<Note>
The web UI hostname **`aws.smith.langchain.com`** is fronted by **Amazon CloudFront**. CloudFront uses a global anycast edge network, so **static destination IP allowlisting is not practical** for that hostname. If you must allowlist fixed IPs, use the **API and gateway hostnames** that terminate on the NLB (and the AWS column above), not the CloudFront URL.
</Note>

You may need to allowlist these to enable traffic from your private network to LangSmith SaaS endpoints (`api.smith.langchain.com`, `smith.langchain.com`, `beacon.langchain.com`, `eu.api.smith.langchain.com`, `eu.smith.langchain.com`, `eu.beacon.langchain.com`, `aws.api.smith.langchain.com`, `aws.smith.langchain.com`, and related `aws.*` API hosts as applicable).
...
<Note>
Private Service Connect is **not** available for the AWS-hosted LangSmith region. Enterprise customers on AWS typically use **static NLB Elastic IPs** (see [Ingress into LangChain SaaS](#ingress-into-langchain-saas)) and TLS to the documented API hostnames.
</Note>
...
The AWS-hosted LangSmith API (`aws.api.smith.langchain.com` and related hosts) uses the **same rate limits as US prod** in the table above unless your contract states otherwise.
