...
5. You will need to enable egress to two control plane URLs. The listener polls these endpoints for deployments (use the pair that matches your LangSmith region—EU: `eu.api.host` / `eu.api.smith`; AWS US SaaS: `aws.api.host` / `aws.api.smith`):
...
4. Configure your `langgraph-dataplane-values.yaml` file.
    ```bash
      config:
        ...
        hostBackendUrl: "https://api.host.langchain.com" # Override for EU (`eu.api.host...`) or AWS US (`aws.api.host...`)
        smithBackendUrl: "https://api.smith.langchain.com" # Override for EU (`eu.api.smith...`) or AWS US (`aws.api.smith...`)
