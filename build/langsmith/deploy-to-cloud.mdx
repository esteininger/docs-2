...
<Note>
**US (AWS):** Organizations on [https://aws.smith.langchain.com](https://aws.smith.langchain.com) use LangGraph Cloud deployments in a dedicated AWS dataplane VPC in `us-east-2`. The addresses in this column are the **LangGraph dataplane** NAT gateway public IPs (one per AZ). They are not the same as the LangSmith control-plane NAT pool in [Cloud allowlisting](/langsmith/cloud#allowlisting-ip-addresses). Your LangChain account team can provide updates, or they are emitted as `lsd_prod_useast2_nat_gateway_public_ips` from internal deployment tooling.
