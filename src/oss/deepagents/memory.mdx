...
- Use **read-only memory** for shared policies. Write the content directly to the store from your application code (e.g., an admin script or startup hook using the [Store API](/langsmith/custom-store)) instead of having the agent create or edit it via `edit_file`, and add a [policy hook](/oss/deepagents/backends#add-policy-hooks) on the backend to reject agent writes to those paths.
