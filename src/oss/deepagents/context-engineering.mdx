...
Runtime context **propagates to all subagents**. When a subagent runs, it receives the same context as the parent. See [Subagents](/oss/deepagents/subagents#context-management) for per-subagent context (namespaced keys).
