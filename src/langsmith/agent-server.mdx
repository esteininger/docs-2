...
[PostgreSQL](https://www.postgresql.org/) is supported as a database for Agent Server and [Redis](https://redis.io/) as the task queue. Checkpoints and long-term memory can optionally be stored using custom adapters, or in MongoDB using our native implementation. See [Configure checkpointer backend](/langsmith/configure-checkpointer) for details.
