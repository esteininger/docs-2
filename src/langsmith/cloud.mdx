...
The [AWS-hosted LangSmith service](https://aws.smith.langchain.com) runs on Amazon EKS in AWS `us-east-2` (Ohio).

<Note>
As of April 2026, LangSmith SaaS is available on AWS in `us-east-2` (Ohio). [LangSmith Cloud deployments](/langsmith/deployment) for that environment use a LangGraph dataplane cluster in the same region; deployment hostnames use `*.aws.us.langgraph.app` (see the [Regions FAQ](/langsmith/regions-faq)).
</Note>
