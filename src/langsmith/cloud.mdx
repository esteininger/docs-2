...
<Note>
**US (AWS):** These are the public IPs of the LangSmith **control-plane** NAT gateways in `us-east-2` (one per availability zone in use). They differ from the LangGraph dataplane NAT pool in [Allowlist IP addresses](/langsmith/deploy-to-cloud#allowlist-ip-addresses). Your LangChain account team can provide updates, or they are emitted as `langsmith_prod_nat_gateway_public_ips` from internal deployment tooling.
</Note>
...
For **egress from LangGraph Cloud deployments** on US (AWS) (traffic leaving the LangGraph dataplane VPC in `us-east-2` toward your endpoints), use the separate NAT allowlist in [Allowlist IP addresses](/langsmith/deploy-to-cloud#allowlist-ip-addresses) (**US (AWS)** column).
...
<Note>
**US (AWS):** Traffic to `aws.api.smith.langchain.com` and related API / gateway hostnames (`aws.api.hub.langchain.com`, `aws.api.host.langchain.com`, etc.) terminates on **two** internet-facing Istio ingress Network Load Balancers in `us-east-2` (primary and weighted routing). Allowlist **all** Elastic IPs above. Your LangChain account team can provide updates, or they are split across `langsmith_prod_istio_gateway_nlb_normal_public_ips` and `langsmith_prod_istio_gateway_nlb_weighted_public_ips` from internal deployment tooling (three EIPs per NLB, one per AZ).
</Note>

<Note>
The web UI hostname **`aws.smith.langchain.com`** is fronted by **Amazon CloudFront**. CloudFront uses a global anycast edge network, so **static destination IP allowlisting is not practical** for that hostname. If you must allowlist fixed IPs, use the **API and gateway hostnames** that terminate on the NLB (and the US (AWS) column above), not the CloudFront URL.
</Note>
