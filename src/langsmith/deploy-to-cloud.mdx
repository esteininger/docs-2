...
| US (GCP)       | EU (GCP)        | AWS LangGraph dataplane (`us-east-2`) |
|----------------|-----------------|---------------------------------------|
| 35.197.29.146  | 34.90.213.236   | _Fill: NAT IP 1_                      |
| 34.145.102.123 | 34.13.244.114   | _Fill: NAT IP 2_                      |
| 34.169.45.153  | 34.32.180.189   | _Fill: NAT IP 3_                      |
...
<Note>
**AWS column:** Organizations on [https://aws.smith.langchain.com](https://aws.smith.langchain.com) use LangGraph Cloud deployments in a dedicated AWS dataplane VPC in `us-east-2`. Replace the placeholders with every address from the LangGraph dataplane NAT gateway pool (one per AZ in use). Your LangChain account team can provide the current list, or they are emitted as `lsd_prod_useast2_nat_gateway_public_ips` from internal deployment tooling.

If you need **ingress** allowlists for traffic **to** deployment URLs under `*.aws.us.langgraph.app`, use the stable NLB Elastic IPs from `lsd_prod_useast2_envoy_gateway_nlb_public_ips` (same tooling). See also [Cloud allowlisting](/langsmith/cloud#allowlisting-ip-addresses) for LangSmith control-plane NAT and API NLB addresses.
</Note>
