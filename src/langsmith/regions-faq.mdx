...
- [https://smith.langchain.com](https://smith.langchain.com) — US (GCP)
- [https://eu.smith.langchain.com](https://eu.smith.langchain.com) — EU (GCP)
- [https://aws.smith.langchain.com](https://aws.smith.langchain.com) — US data residency on AWS (`us-east-2`)

API endpoints follow the same pattern: `api.smith.langchain.com`, `eu.api.smith.langchain.com`, and `aws.api.smith.langchain.com`. [LangSmith Cloud deployments](/langsmith/deployment) use `*.us.langgraph.app`, `*.eu.langgraph.app`, and `*.aws.us.langgraph.app` respectively. See the [cloud architecture reference](/langsmith/cloud#architecture) for details.
